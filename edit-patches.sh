...
prompt_changes() {
	bold_arrow; echo "Editing $COLLECTIONS_COMMIT"
	...
	patch -s -p1 < $PATCH_DIR/$COLLECTIONS_COMMIT.patch
	git commit -a -m "existing patch for $COLLECTIONS_COMMIT" > /dev/null
	...
	bold_arrow; echo "Replacing $COLLECTIONS_COMMIT.patch with updated version"
	git diff > $TMP_PATCH
	git clean -f -x
	git diff > $PATCH_DIR/$COLLECTIONS_COMMIT.patch
	rm -rf .git
}
...
for COLLECTIONS_COMMIT in $(git_commits_ordered %H $PATCH_COMMITS|tac); do
	prepare_version
